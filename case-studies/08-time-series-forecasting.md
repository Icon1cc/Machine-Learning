...
Build a time series forecasting for demand, traffic, or resource planning. The system uses timestamped observations, calendar features, promotions, outages, and external drivers to support this output:
forecast with prediction intervals and anomaly flags. Treat the case as an interview design exercise and a production review: define the
...
| --- | --- |
| Primary quality | MAE, WAPE, interval coverage, bias by segment, and business planning error |
...
- The critical failure to plan around is missing a demand spike that causes stockouts or capacity incidents.
