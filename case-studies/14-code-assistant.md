...
Build a code assistant for developer productivity. The system uses code context, repository metadata, tests, user request, and tool outputs to support this output:
patch, explanation, test command, or code review finding. Treat the case as an interview design exercise and a production review: define the
...
| --- | --- |
| Primary quality | accepted changes, build pass rate, defect rate, latency, and developer review effort |
...
- The critical failure to plan around is modifying unrelated code or introducing a hidden security regression.
