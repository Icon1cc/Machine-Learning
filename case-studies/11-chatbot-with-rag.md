...
Build a chatbot with RAG for grounded question answering. The system uses user question, conversation state, retrieved passages, source metadata, and safety context to support this output:
answer with citations, uncertainty, and escalation when evidence is missing. Treat the case as an interview design exercise and a production review: define the
...
will recover when confidence is low. The highest-risk failure to plan around is presenting an unsupported answer as if it came from the source material.

## Functional Requirements

- Ingest user question, conversation state, retrieved passages, source metadata, and safety context.
- Produce answer with citations, uncertainty, and escalation when evidence is missing.
...
After measuring the baseline, consider hybrid retrieval, reranking, context compression, and answer grounding checks. Add complexity only when it improves a named
...
Evaluate with answer faithfulness, citation precision, user resolution rate, latency, and refusal quality. Include slice analysis for important user, item, time, source, language, or
...
| --- | --- |
| Primary quality | answer faithfulness, citation precision, user resolution rate, latency, and refusal quality |
...
- The critical failure to plan around is presenting an unsupported answer as if it came from the source material.
...
presenting an unsupported answer as if it came from the source material. Then I would build keyword retrieval plus extractive answer snippets, define metrics around answer faithfulness, citation precision, user resolution rate, latency, and refusal quality, inspect errors by segment,
and only then consider hybrid retrieval, reranking, context compression, and answer grounding checks. For production, I would add monitoring, fallback behavior, privacy
...
[⬅ Previous](10-semantic-search-engine.md) | [🏠 Home](../README.md) | [➡ Next](12-enterprise-knowledge-assistant.md)
