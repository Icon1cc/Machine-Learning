...
Design a Search Ranking System is about designing a reliable workflow for a user searching a product, document, or content corpus. The model is only one part of the design.
The system must collect trustworthy data, produce the expected output (ranked results with snippets, filters, scores, and trace metadata), serve it within
...
- User and decision: a user searching a product, document, or content corpus.
- Expected output: ranked results with snippets, filters, scores, and trace metadata.
...
- Data includes queries, documents, metadata, clicks, skips, judgments, freshness, and permissions.
...
GET /search?q=&filters=&user_id= returns ranked documents, snippets, facets, and trace id.
...
| --- | --- | --- |
| Decision logic | BM25 with field boosts, query normalization, and permission or catalog filters | hybrid retrieval with vector search, learned ranking features, and reranking |
...
A realistic first version would ship BM25 with field boosts, query normalization, and permission or catalog filters. The team would measure NDCG, MRR, zero-result rate, latency, abandonment, and judged relevance, inspect failures,
and only then move toward hybrid retrieval with vector search, learned ranking features, and reranking. This keeps the design honest: model complexity is justified by
...
- Monitor NDCG, MRR, zero-result rate, latency, abandonment, and judged relevance plus technical health such as error rate, queue depth, and p95 latency.
...
The primary failure to plan around is ranking stale, inaccessible, or semantically similar but wrong documents above the answer. Other common bottlenecks include delayed labels,
...
[⬅ Previous](01-design-a-recommendation-system.md) | [🏠 Home](../README.md) | [➡ Next](03-design-a-fraud-detection-platform.md)
