...
Build a recommendation system for personalized content or product discovery. The system uses user events, item metadata, inventory state, impressions, clicks, purchases, and negative feedback to support this output:
ranked candidates with reasons and diversity controls. Treat the case as an interview design exercise and a production review: define the
...
will recover when confidence is low. The highest-risk failure to plan around is over-personalizing into a narrow loop that hurts discovery and trust.

## Functional Requirements

- Ingest user events, item metadata, inventory state, impressions, clicks, purchases, and negative feedback.
- Produce ranked candidates with reasons and diversity controls.
...
After measuring the baseline, consider two-stage retrieval and ranking with embeddings, learning-to-rank features, and exploration. Add complexity only when it improves a named
...
Evaluate with CTR, conversion, retention, diversity, freshness, coverage, and long-term satisfaction. Include slice analysis for important user, item, time, source, language, or
...
| --- | --- |
| Primary quality | CTR, conversion, retention, diversity, freshness, coverage, and long-term satisfaction |
...
- The critical failure to plan around is over-personalizing into a narrow loop that hurts discovery and trust.
...
over-personalizing into a narrow loop that hurts discovery and trust. Then I would build popular items, recent items, collaborative filtering, and simple content similarity, define metrics around CTR, conversion, retention, diversity, freshness, coverage, and long-term satisfaction, inspect errors by segment,
and only then consider two-stage retrieval and ranking with embeddings, learning-to-rank features, and exploration. For production, I would add monitoring, fallback behavior, privacy
