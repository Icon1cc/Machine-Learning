...
Build an AI agent for workflows for workflow automation. The system uses user goal, available tools, permissions, task state, documents, and approval policy to support this output:
completed task, plan, tool trace, or escalation request. Treat the case as an interview design exercise and a production review: define the
...
| --- | --- |
| Primary quality | task success rate, intervention rate, unsafe action rate, latency, and auditability |
...
- The critical failure to plan around is calling a tool that changes state without the required approval.
