...
Build an AI meeting summarizer for meeting notes and action tracking. The system uses transcript, speaker turns, calendar metadata, chat messages, and organization policy to support this output:
summary, decisions, action items, owners, deadlines, and uncertainty markers. Treat the case as an interview design exercise and a production review: define the
...
will recover when confidence is low. The highest-risk failure to plan around is assigning an action to the wrong person or exposing confidential content.

## Functional Requirements

- Ingest transcript, speaker turns, calendar metadata, chat messages, and organization policy.
- Produce summary, decisions, action items, owners, deadlines, and uncertainty markers.
...
After measuring the baseline, consider LLM summarization with entity resolution, action extraction, and human correction feedback. Add complexity only when it improves a named
...
Evaluate with action-item precision, decision recall, edit distance from human notes, privacy compliance, and latency. Include slice analysis for important user, item, time, source, language, or
...
| --- | --- |
| Primary quality | action-item precision, decision recall, edit distance from human notes, privacy compliance, and latency |
...
- The critical failure to plan around is assigning an action to the wrong person or exposing confidential content.
...
assigning an action to the wrong person or exposing confidential content. Then I would build extractive notes using transcript sections and keyword action detection, define metrics around action-item precision, decision recall, edit distance from human notes, privacy compliance, and latency, inspect errors by segment,
and only then consider LLM summarization with entity resolution, action extraction, and human correction feedback. For production, I would add monitoring, fallback behavior, privacy
...
[⬅ Previous](14-code-assistant.md) | [🏠 Home](../README.md) | [➡ Next](16-llm-evaluation-platform.md)
