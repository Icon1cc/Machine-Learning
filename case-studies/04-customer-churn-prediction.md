...
Build a customer churn prediction for subscription retention. The system uses usage history, billing events, support tickets, plan changes, tenure, and engagement trends to support this output:
churn probability, risk segment, and suggested retention action. Treat the case as an interview design exercise and a production review: define the
...
will recover when confidence is low. The highest-risk failure to plan around is targeting users who would have stayed without intervention and wasting retention budget.

## Functional Requirements

- Ingest usage history, billing events, support tickets, plan changes, tenure, and engagement trends.
- Produce churn probability, risk segment, and suggested retention action.
...
After measuring the baseline, consider survival analysis or gradient boosting with time-windowed behavioral features. Add complexity only when it improves a named
...
Evaluate with lift in contacted segments, calibration, retention impact, intervention cost, and customer experience guardrails. Include slice analysis for important user, item, time, source, language, or
...
| --- | --- |
| Primary quality | lift in contacted segments, calibration, retention impact, intervention cost, and customer experience guardrails |
...
- The critical failure to plan around is targeting users who would have stayed without intervention and wasting retention budget.
...
targeting users who would have stayed without intervention and wasting retention budget. Then I would build recency and usage rules plus logistic regression on account-level features, define metrics around lift in contacted segments, calibration, retention impact, intervention cost, and customer experience guardrails, inspect errors by segment,
and only then consider survival analysis or gradient boosting with time-windowed behavioral features. For production, I would add monitoring, fallback behavior, privacy
...
[⬅ Previous](03-credit-risk-model.md) | [🏠 Home](../README.md) | [➡ Next](05-recommendation-system.md)
