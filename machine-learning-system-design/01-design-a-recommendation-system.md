...
Design a Recommendation System is about designing a reliable workflow for a consumer app user opening a personalized home surface. The model is only one part of the design.
The system must collect trustworthy data, produce the expected output (a ranked list of items with diversity, freshness, and safety constraints), serve it within
...
- User and decision: a consumer app user opening a personalized home surface.
- Expected output: a ranked list of items with diversity, freshness, and safety constraints.
...
- Data includes impressions, clicks, dwell time, hides, follows, item metadata, creator features, and freshness.
...
GET /recommendations?user_id=&surface=&limit= returns ranked item ids, scores, reasons, and trace id.
...
| --- | --- | --- |
| Decision logic | popular, recent, and followed-source candidates with simple engagement scoring | two-stage candidate generation and learning-to-rank with exploration and reranking |
...
A realistic first version would ship popular, recent, and followed-source candidates with simple engagement scoring. The team would measure NDCG, CTR, long-term retention, diversity, freshness, and negative feedback rate, inspect failures,
and only then move toward two-stage candidate generation and learning-to-rank with exploration and reranking. This keeps the design honest: model complexity is justified by
...
- Monitor NDCG, CTR, long-term retention, diversity, freshness, and negative feedback rate plus technical health such as error rate, queue depth, and p95 latency.
...
The primary failure to plan around is a feedback loop that over-optimizes short-term clicks and narrows discovery. Other common bottlenecks include delayed labels,
...
[⬅ Previous](../production-ai/12-building-enterprise-ai-systems.md) | [🏠 Home](../README.md) | [➡ Next](02-design-a-search-ranking-system.md)
