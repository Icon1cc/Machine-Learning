...
Build a customer support agent for support automation. The system uses ticket text, account state, policy documents, prior cases, tools, and escalation rules to support this output:
draft response, next action, or routed ticket with confidence and evidence. Treat the case as an interview design exercise and a production review: define the
...
will recover when confidence is low. The highest-risk failure to plan around is taking an account action without enough evidence or authorization.

## Functional Requirements

- Ingest ticket text, account state, policy documents, prior cases, tools, and escalation rules.
- Produce draft response, next action, or routed ticket with confidence and evidence.
...
After measuring the baseline, consider tool-using agent with constrained actions, policy checks, and human approval gates. Add complexity only when it improves a named
...
Evaluate with first-contact resolution, escalation accuracy, handle time, CSAT, and safety incidents. Include slice analysis for important user, item, time, source, language, or
...
| --- | --- |
| Primary quality | first-contact resolution, escalation accuracy, handle time, CSAT, and safety incidents |
...
- The critical failure to plan around is taking an account action without enough evidence or authorization.
...
taking an account action without enough evidence or authorization. Then I would build intent classifier, macro templates, and retrieval over support articles, define metrics around first-contact resolution, escalation accuracy, handle time, CSAT, and safety incidents, inspect errors by segment,
and only then consider tool-using agent with constrained actions, policy checks, and human approval gates. For production, I would add monitoring, fallback behavior, privacy
...
[⬅ Previous](12-enterprise-knowledge-assistant.md) | [🏠 Home](../README.md) | [➡ Next](14-code-assistant.md)
