...
Build a document classification for document routing and compliance triage. The system uses document text, layout metadata, source, language, and human labels to support this output:
document class, confidence, and fields required for downstream routing. Treat the case as an interview design exercise and a production review: define the
...
will recover when confidence is low. The highest-risk failure to plan around is misrouting a regulated document that requires special handling.

## Functional Requirements

- Ingest document text, layout metadata, source, language, and human labels.
- Produce document class, confidence, and fields required for downstream routing.
...
After measuring the baseline, consider layout-aware or transformer classifiers with active learning for uncertain classes. Add complexity only when it improves a named
...
Evaluate with macro F1, per-class recall, review load, routing latency, and audit error rate. Include slice analysis for important user, item, time, source, language, or
...
| --- | --- |
| Primary quality | macro F1, per-class recall, review load, routing latency, and audit error rate |
...
- The critical failure to plan around is misrouting a regulated document that requires special handling.
...
misrouting a regulated document that requires special handling. Then I would build rules and TF-IDF linear models with human review for low confidence, define metrics around macro F1, per-class recall, review load, routing latency, and audit error rate, inspect errors by segment,
and only then consider layout-aware or transformer classifiers with active learning for uncertain classes. For production, I would add monitoring, fallback behavior, privacy
...
[⬅ Previous](08-time-series-forecasting.md) | [🏠 Home](../README.md) | [➡ Next](10-semantic-search-engine.md)
