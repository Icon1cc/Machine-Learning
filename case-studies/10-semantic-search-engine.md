...
Build a semantic search engine for meaning-based knowledge retrieval. The system uses queries, documents, metadata, embeddings, permissions, and click or judgment feedback to support this output:
ranked passages or documents with matched evidence. Treat the case as an interview design exercise and a production review: define the
...
will recover when confidence is low. The highest-risk failure to plan around is retrieving semantically similar but factually wrong evidence.

## Functional Requirements

- Ingest queries, documents, metadata, embeddings, permissions, and click or judgment feedback.
- Produce ranked passages or documents with matched evidence.
...
After measuring the baseline, consider dense retrieval, hybrid search, reranking, and query rewriting. Add complexity only when it improves a named
...
Evaluate with recall at k, MRR, judged relevance, latency, and permission violation rate. Include slice analysis for important user, item, time, source, language, or
...
| --- | --- |
| Primary quality | recall at k, MRR, judged relevance, latency, and permission violation rate |
...
- The critical failure to plan around is retrieving semantically similar but factually wrong evidence.
...
retrieving semantically similar but factually wrong evidence. Then I would build BM25 plus metadata filters and manually tuned synonyms, define metrics around recall at k, MRR, judged relevance, latency, and permission violation rate, inspect errors by segment,
and only then consider dense retrieval, hybrid search, reranking, and query rewriting. For production, I would add monitoring, fallback behavior, privacy
...
[⬅ Previous](09-document-classification.md) | [🏠 Home](../README.md) | [➡ Next](11-chatbot-with-rag.md)
