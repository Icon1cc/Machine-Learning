...
Design a RAG Platform is about designing a reliable workflow for employees or customers asking questions over private and changing documents. The model is only one part of the design.
The system must collect trustworthy data, produce the expected output (grounded answer with citations, abstention, permissions, and trace id), serve it within
...
- User and decision: employees or customers asking questions over private and changing documents.
- Expected output: grounded answer with citations, abstention, permissions, and trace id.
...
- Data includes documents, chunks, ACLs, metadata, embeddings, queries, answers, citations, and feedback.
...
POST /rag/query accepts question and user context and returns answer, citations, abstention, and trace id.
...
| --- | --- | --- |
| Decision logic | permission-filtered keyword search with snippets and source links | hybrid retrieval, reranking, context compression, cite-or-abstain generation, and RAG evaluation |
...
A realistic first version would ship permission-filtered keyword search with snippets and source links. The team would measure retrieval recall, context precision, faithfulness, citation accuracy, p95 latency, and access violations, inspect failures,
and only then move toward hybrid retrieval, reranking, context compression, cite-or-abstain generation, and RAG evaluation. This keeps the design honest: model complexity is justified by
...
- Monitor retrieval recall, context precision, faithfulness, citation accuracy, p95 latency, and access violations plus technical health such as error rate, queue depth, and p95 latency.
...
The primary failure to plan around is retrieving restricted or stale content, or generating an unsupported answer with confident wording. Other common bottlenecks include delayed labels,
...
[⬅ Previous](05-design-a-feature-store.md) | [🏠 Home](../README.md) | [➡ Next](07-design-an-agent-platform.md)
