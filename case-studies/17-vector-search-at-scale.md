...
Build a vector search at scale for large-scale approximate nearest neighbor retrieval. The system uses embeddings, metadata, index versions, query vectors, filters, and relevance judgments to support this output:
nearest candidates with scores, filters, and index trace metadata. Treat the case as an interview design exercise and a production review: define the
...
will recover when confidence is low. The highest-risk failure to plan around is reducing recall during an index migration without detection.

## Functional Requirements

- Ingest embeddings, metadata, index versions, query vectors, filters, and relevance judgments.
- Produce nearest candidates with scores, filters, and index trace metadata.
...
After measuring the baseline, consider sharded HNSW or IVF-PQ with hybrid filtering, reranking, and versioned rollouts. Add complexity only when it improves a named
...
Evaluate with recall at k, p95 latency, index build time, memory use, and freshness. Include slice analysis for important user, item, time, source, language, or
...
| --- | --- |
| Primary quality | recall at k, p95 latency, index build time, memory use, and freshness |
...
- The critical failure to plan around is reducing recall during an index migration without detection.
...
reducing recall during an index migration without detection. Then I would build exact search on a sample plus a simple HNSW index, define metrics around recall at k, p95 latency, index build time, memory use, and freshness, inspect errors by segment,
and only then consider sharded HNSW or IVF-PQ with hybrid filtering, reranking, and versioned rollouts. For production, I would add monitoring, fallback behavior, privacy
...
[⬅ Previous](16-llm-evaluation-platform.md) | [🏠 Home](../README.md) | [➡ Next](18-ml-monitoring-platform.md)
