...
Build a spam classifier for email and message moderation. The system uses message body, subject, sender reputation, links, attachments, and user reports to support this output:
spam, likely spam, or clean with a calibrated confidence score. Treat the case as an interview design exercise and a production review: define the
...
will recover when confidence is low. The highest-risk failure to plan around is blocking a legitimate account recovery or business email.

## Functional Requirements

- Ingest message body, subject, sender reputation, links, attachments, and user reports.
- Produce spam, likely spam, or clean with a calibrated confidence score.
...
After measuring the baseline, consider gradient boosting over engineered sender signals plus a transformer text classifier for hard examples. Add complexity only when it improves a named
...
Evaluate with precision at high recall, false-positive rate on trusted senders, review volume, and complaint rate. Include slice analysis for important user, item, time, source, language, or
...
| --- | --- |
| Primary quality | precision at high recall, false-positive rate on trusted senders, review volume, and complaint rate |
...
blocking a legitimate account recovery or business email. Then I would build keyword rules, deny lists, sender reputation checks, and a logistic regression model over TF-IDF features, define metrics around precision at high recall, false-positive rate on trusted senders, review volume, and complaint rate, inspect errors by segment,
and only then consider gradient boosting over engineered sender signals plus a transformer text classifier for hard examples. For production, I would add monitoring, fallback behavior, privacy
...
[⬅ Previous](../ethics-safety/07-ai-governance.md) | [🏠 Home](../README.md) | [➡ Next](02-fraud-detection.md)
