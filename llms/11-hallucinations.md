...
Hallucinations is part of the larger skill of using data to make better decisions. At a beginner level,
...
In formal ML work, hallucinations should be described through inputs, outputs, assumptions, and an
objective. For this lesson, the core definition is: describe unsupported or fabricated model claims. A rigorous explanation also names the
data distribution, the parameters or rules being learned, and the metric used to judge generalization.

Key pieces:

- Core idea: describe unsupported or fabricated model claims.
- Input: the data, assumptions, or constraints that make hallucinations meaningful.
...
When you use hallucinations at work, you should be able to explain what can go wrong, what you would
...
A sales assistant drafts account summaries from CRM notes, retrieves relevant facts, and asks for confirmation before writing back. In that setting, hallucinations helps the team move from a vague request to a
...
- Using hallucinations because it sounds advanced instead of because it matches the problem.
...
- Memorizing the definition of hallucinations without being able to trace the data flow.

## Interview Angle

Interviewers use hallucinations to test whether you understand fundamentals rather than memorized
phrases.

**Question:** Explain hallucinations to a product manager and then to a senior ML engineer.
...
Pick a product you use every week. Write down one place where hallucinations could appear. Define
